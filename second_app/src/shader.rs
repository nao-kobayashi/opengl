...
impl Shader {
    ...
    unsafe fn check_compile_errors(&self, shader: u32, type_: &str) {
        ...
        if type_ != "PROGRAM" {
            ...
            if success != gl::TRUE as GLint {
                ...
                let info_log_string = String::from_utf8_lossy(&info_log);
                println!(
    ...
    pub fn with_geometory_shader (
        ...
        let mut fragment_file = File::open(fragment_path)
            .unwrap_or_else(|_| panic!("failed to open file {}", fragment_path));
        let mut geometory_file = File::open(geometory_path)
            .unwrap_or_else(|_| panic!("failed to open file {}", geometory_path));

        let mut vertex_code = String::new();
        let mut fragment_code = String::new();
        let mut geometory_code = String::new();

        vertex_file
            .read_to_string(&mut vertex_code)
            .expect("failed to read vertex sharder file");
        ...
        geometory_file
            .read_to_string(&mut geometory_code)
            .expect("failed to read geometory shader file");

        let cstr_vertex_code = CString::new(vertex_code.as_bytes()).unwrap();
        let cstr_fragment_code = CString::new(fragment_code.as_bytes()).unwrap();
        let cstr_geometory_code = CString::new(geometory_code.as_bytes()).unwrap();
        ...
        unsafe {
            ...
            let geometory = gl::CreateShader(gl::GEOMETRY_SHADER);
            gl::ShaderSource(geometory, 1, &cstr_geometory_code.as_ptr(), ptr::null());
            gl::CompileShader(geometory);
            shader.check_compile_errors(geometory, "GEOMETORY");


            let id = gl::CreateProgram();
            gl::AttachShader(id, vertex);
            gl::AttachShader(id, fragment);
            gl::AttachShader(id, geometory);
            gl::LinkProgram(id);
            shader.check_compile_errors(id, "PROGRAM");

            gl::DeleteShader(vertex);
            gl::DeleteShader(fragment);
            gl::DeleteShader(geometory);
            shader.id = id;
        }
